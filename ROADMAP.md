...
`DESIGN.md` 본문 분리는 아직 안 했다 → `docs/PENDING.md` 미결
...
⚠️ **잔여 — `StrengthMultiplier` 미배선** (2026-08-28 조사에서 발견).

위 **[확정됨]** 이 "환생 배수는 획득량에 곱한다. 결합은 `StrengthMultiplier.compute`가
맡는다"고 적고 있으나 **배선되지 않았다.** `compute`의 호출자가 코드 전체에 0이고
`ConfigTests`가 require할 뿐이다. `ClickService.computeGain`(`:167`)은
`padPower × accepted` 곱셈만 하고 배수가 없다 —
**지금 환생을 100번 해도 클릭당 힘은 그대로다.**

`SpeedService.onRebirth`가 호출자 없이 남아 있던 것과 같은 종류의 사고다.
배수가 안 붙어도 힘은 멀쩡히 오르므로 플레이로는 발견되지 않는다.
규모는 한 줄~몇 줄. → `docs/PENDING.md`

이 절의 ✅ 완료 표기는 유지한다 — 커밋과 Play 검증은 실제로 끝났고,
빠진 것은 그 위에 얹혀야 할 배선 한 줄이다.
...
##### 구현 결과 (2026-08-28). ⚠️ Play 미검증 — RC 환경
...
⚠️ 배선 전체가 `ATTACK_WIRING_ENABLED`(기본 `true`)로 감싸여 있다. 서비스와 배선이
한 커밋에 있어 되돌릴 단위가 없기 때문이다 → `docs/PENDING.md` 잔재.
