...
| 2026-08-28 | `DESIGN.md` 펀치 속도 절 분리 (클릭 파워 패드 절에 수동 클릭 상한과 나란히 있었다) | `TBD2` — `DESIGN.md` "펀치와 딜 총량" 신설. 딜 공식도 함께 옮김 |
| 2026-08-28 | 자동 진행 안전층 정의 조정 (거리 폴링 확정으로 `D`가 이론 상한이 됐다) | `TBD2` — 안전층 = "반경 안에 계속 있었을 때 클리어 가능한 최대 층"으로 정정. "자동 진행 모드" 절의 "조작 편차 없는 확정값" 서술 제거 |
