...
- **17~25층 세그먼트 7.0** — 정적 검토 완료 (2026-08-26). 1순위 후보 **9.0, 미적용.**
  ...
  ⚠️ **17층 절벽은 2026-08-29 실측으로 기각됐다(해소 기록 참고). 그렇다고 9.0이
  승인된 것이 아니다.** 절벽이 없다는 것은 "조여도 특정 층이 터지지 않는다"이지
  "조여도 된다"가 아니다. 진행 속도 전체가 얼마나 느려지는가는 따로 재야 한다.

  판단 방법: `StandardPathReport`가 이미 있으므로 `WorldConfig`의 세 번째
  세그먼트를 9.0으로 바꿔 재실행하면 25층 도달 시간이 나온다. **그 증가폭이 판단 근거다.**
  (리포트 자체는 손대지 않는다 — Config 값만 바꾸고 돌린다)
...
- **`result=out_of_range`를 실물에서 보지 못했다** — 2026-08-28 Play 이후 남은 관측 구멍.
  ...
  ⚠️ **런이 끝난 뒤에 나가면 안 된다.** 2026-08-29 Play에서 패드 2를 밟긴 했으나
  런이 이미 끝난 뒤라 `no_run`으로 찍혔다. 그 줄은 반경 판정을 거치지 않으므로
  `out_of_range` 관측이 되지 않는다. 여전히 미관측이다.
