...
- **사라진 PENDING 절을 가리키는 포인터 3곳** — 2026-08-28 발견. 미수정 2곳.
  `2e1d4b3`에서 "지환 쪽" 절을 재구성하면서 `신규 — 수령 발판 · 진행 벽 최소 깊이`라는
  절 이름이 없어졌다. 내용은 `docs/UI_ASSET_SPEC.md` "5-1"로 갔다.

  ```
  ROADMAP.md                        고침 (이번 커밋)
  src/shared/Config/AttackConfig.lua  ⚠️ 미수정 — 코드 파일이라 열지 않았다
  src/shared/Config/LevelConfig.lua   ⚠️ 미수정 — "HANDOFF"로 적혀 있어 원래도 틀렸다
  ```

  ⚠️ 주석 한 줄이라 급하지 않지만 **다음에 그 파일을 여는 작업에서 함께 고칠 것.**
  지금 고치면 Play 미검증 레이어가 하나 더 생긴다(`AttackConfig`는 `d153228` 미검증).
