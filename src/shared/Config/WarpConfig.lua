...
-- ===== 튜닝 수치 =====================================================================
--
-- ⚠️ TEMP — 4-2-f 실측 튜닝 대상. bloxBase = 1과 같은 성격이 **아니다** —
-- 그쪽은 확정값으로 종료됐지만 이 둘은 아직 아니다. TEMP 접두어를 계속 유지한다.
--
-- ✅ 2026-08-30: BASE만 100 → 10으로 조정. **최종 확정이 아니라 "사문을 벗어난 자리"다.**
-- RATIO는 그대로(3.0).
--
-- 근거 (WarpConversionReport 실측, `src/server/Tests/WarpConversionReport.server.lua`):
--   BASE = 100이었을 때는 어느 층이든 그 층 100판 이상이라는 바닥이 깔려 있었다.
--   워프의 용도가 환생 후 복귀인데 100판이면 전 구간에서 다시 걸어 올라가는 쪽이
--   싸다 — 사치품이 아니라 사실상 못 쓰는 사문이었다. 25층 워프는 28.24T였고
--   그 시점 lifetimeBlox(141.53B)의 200배였다.
--   BASE = 10으로 내리면 판어치가 10(1층) → 125(25층)로 내려온다. 25층 비용은
--   2.82T, 그 층 1판(22.53B) 기준 약 125판 ≈ 42분 — 사치품이되 도달 가능한 자리다.
--
--   RATIO는 건드리지 않았다. 정상 진행의 하한(보상 성장률 2.7)보다 위에 있고,
--   25층 누적 12.5배가 "후반이 상대적으로 비싸진다"는 의도를 이미 만들고 있다.
--   두 값을 함께 움직이면 후반 비용이 눈에 보이는 것보다 훨씬 크게 튄다(아래 참고).
--
-- 자리값을 고른 근거(BASE는 위 실측, RATIO는 애초 근거 — "왜 이 자리인지"):
--   BASE  = 10  — 1층 워프가 1층 보상 10판어치. bloxBase = 1 위에 얹은 값이라
...
--    (3.0/2.7)^24 ≈ 12.7배 — 1층에서 10판어치였던 것이 25층에서는 125판어치가 된다.
--    이게 의도(후반 워프는 사치품)인지 과한지는 4-2-f에서 볼 문제다.
--    여기서는 "이 비율차가 누적된다"는 사실만 남긴다. 두 값 중 하나만 건드리면
--    후반 비용이 눈에 띄는 것보다 훨씬 크게 움직인다.
--
-- ⚠️ BASE를 내려도 워프는 여전히 환생과의 지갑 경쟁에서 구조적으로 진다 —
--    환생은 절대 단가(blox/1000)이고 워프는 지수라, 같은 지갑에서 후반으로 갈수록
--    환생이 무조건 이긴다. TEMP 두 값 튜닝으로는 못 고치는 문제다 (→ docs/PENDING.md).
local TEMP_COST_BASE = 10
