...
local Worlds: { [number]: WorldDef } = {
	[1] = {
		...
		eggPacks = { 1, 2, 3 },
		auraPacks = { 1, 2, 3, 4, 5 },
		-- 클릭 파워 패드 24개. 파워는 패드당 2배(2 → 16.78M), 해금 조건은 패드당 3배로
		-- 파워보다 가파르다 — 뒤 패드일수록 "한 칸 더"의 대가가 커져야 방치 시간이 의미를 갖는다.
		--
		-- ⚠️ TEMP — basePower는 4-2-f 실측 튜닝 대상이다. 확정값이 아니다.
		--
		-- ✅ 2026-08-30: 1 → 2. `StandardPathReport` 파워 스윕(기준값의 0.5/1/2/5/10배 ×
		-- 클릭률 3종) 실측 결과. ⚠️ 이 판단은 시간 축이지 성공률 검증이 아니다 — 확률적
		-- 플레이어 모델이 없어 25층 도달 시각·1층 소요초로만 골랐다.
		--   1배는 1층 소요 7.0초 — 방치형 첫인상 구간에서 너무 느리다.
		--   2배는 1층 5.0초 / 여유배수 15.73 — 튜토리얼 층으로 넉넉하다.
		--   5배·10배는 반대로 무너진다. 10배는 1층 2.1초 / 25층 18.4분이지만
		--   도달 시점 패드가 21로 내려가 후반 성장 목표(패드 23·24)가 사라진다.
		--   2배가 유지하는 것: 25층 41.1분, 도달 시점 패드 22(1배와 동일),
		--   17층 여유배수 1.1~1.3.
		-- ⚠️ 17층 여유배수는 배수와 무관하게 1.1~2.6으로 흩어져 단조성이 없었다 —
		-- 표준 플레이어가 정지선 규칙으로 늘 한계층에 붙어 있어, 파워가 세져도
		-- "같은 성공률에 더 빨리 도착"할 뿐이기 때문이다. 목표 성공률(1~8층 95% /
		-- 9~16층 65~85% / 17~25층 40~55%)에 드는지는 이번에 확인하지 않았다
		-- (→ docs/PENDING.md "basePower 2는 시간 축 판단이다").
		clickPadSet = {
			count = 24,
			basePower = BigNum.new(2, 0), -- 2
			powerGrowth = 2.0,
