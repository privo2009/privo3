...
--
-- ⚠️ 반드시 BigNum으로 곱한다. 패드 24는 파워가 수백만~천만 단위(basePower ×
-- powerGrowth^23 — basePower는 4-2-f 튜닝 대상이라 정확한 값이 움직인다)이고
-- 환생·게임패스 배수가 붙으면 그 위로 더 간다. raw number로 곱하면 언젠가 조용히
-- 정밀도가 깨진다 (CLAUDE.md 1).
--
